...
const { buildItrootsCareerCounsellingFlow } = require('../src/utils/itrootsFlowTemplate');
...
test('Finlec Technologies counselling flow uses dynamic business placeholders in follow-up messages', () => {
  const { flowData } = buildItrootsCareerCounsellingFlow();
...
test('Finlec Technologies counselling flow routes call and website quick replies into follow-up message nodes', () => {
  const { flowData } = buildItrootsCareerCounsellingFlow();
  ...
  assert.deepEqual(feeButtons, ['Book Demo', 'Call Counsellor', 'View Website']);
  assert.deepEqual(admissionButtons, ['Book Demo', 'Call Counsellor', 'View Website']);
  assert.deepEqual(confirmButtons, ['Call Counsellor', 'View Website']);
